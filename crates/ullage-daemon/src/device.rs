...
use crate::privatefs::{self, ErrorStyle, FileStrictness};
...
impl DeviceStore {
    ...
    pub fn open(path: impl Into<PathBuf>) -> Result<Self, String> {
        ...
        privatefs::ensure_private_parent(&path, ErrorStyle::Path)?;
        privatefs::sweep_stale_temporary_files(&path);
        let devices =
            match privatefs::read_private_file(&path, FileStrictness::Exact, ErrorStyle::Path)? {
                Some(bytes) => {
...
fn write_device_file(path: &Path, devices: &[DeviceRecord]) -> Result<(), String> {
    ...
    privatefs::replace_private_file(path, &bytes, ErrorStyle::Path)
}
