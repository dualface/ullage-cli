...
use ullage_cli::{
    ClientError, ControlClient, ExitCode, OutputFormat, ServiceAction, run_from, run_from_with,
};
use ullage_protocol::{
...
impl MockClient {
    ...
    fn with_daemon_result(daemon_result: Result<(), ClientError>) -> Self {
        Self {
...
fn empty_usage() -> SubscriptionUsage {
    SubscriptionUsage {
...
fn snapshot(account_id: &str, usage: QueryOutcome<SubscriptionUsage>) -> SnapshotPayload {
    SnapshotPayload {
...
fn snapshot_result(request: &ControlRequest) -> Result<ControlResponse, ClientError> {
    Ok(response(
        request,
...
fn account_list_result(request: &ControlRequest) -> Result<ControlResponse, ClientError> {
    Ok(response(
        request,
        ControlResult::Accounts(vec![Account {
            id: AccountId::new("primary"),
...
fn summarizable_usage() -> SubscriptionUsage {
    let now = Utc::now();
    let mut usage = empty_usage();
...
fn summary_result(request: &ControlRequest) -> Result<ControlResponse, ClientError> {
    Ok(response(
        request,
        ControlResult::Snapshots(vec![snapshot(
            "primary",
...
fn line_starting_with<'a>(stdout: &'a str, prefix: &str) -> &'a str {
    stdout
        .lines()
...
fn partial_profile_incompatible(request: &ControlRequest) -> Result<ControlResponse, ClientError> {
    Ok(response(
        request,
        ControlResult::Snapshots(vec![snapshot(
            "primary",
...
/// The stdout block for one account in a `show --all` output.
fn account_block<'a>(stdout: &'a str, account: &str) -> &'a str {
...
fn metric_usage() -> SubscriptionUsage {
    let mut usage = empty_usage();
    usage.windows = vec![
        UsageWindow {
            window: UsageWindowKind::FiveHours,
