...
该视图每两分钟重新读取一次快照——守护进程每五分钟探测一次，这个间隔能接住每一轮而不会空跑。状态行倒计时到下一次读取：一条十格方块条，初始满格，随时间逐格熄灭，正在走的那一格再按八分之一细分，所以每秒都在动；方块条旁边是整秒的等待时间（`2m00s`、`0m45s`）。无论方框怎么排，状态行都居中显示——方框居中一行与铺满宽度的一行，它都落在同一个位置——它与方框之间空一行，屏幕最上方也留一个空行。守护进程答不上来时，表现就是方块条走空后停在那里：屏幕上的读数保持不变，下一个间隔再试。
